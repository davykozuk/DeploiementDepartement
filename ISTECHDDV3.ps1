...
# Variables globales
$checkboxes = @()
$script:detailedLog = @() # Log détaillé pour le fichier
...
function Write-Log {
    param (
        ...
        [switch]$FileOnly # N'affiche que dans le fichier, pas dans l'interface
    )
    ...
    # Ajout au log détaillé (fichier)
    $script:detailedLog += $formattedMessage
    
    # Affichage dans l'interface seulement si pas FileOnly
    if (-not $FileOnly) {
...
function Start-SoftwareInstallation {
    ...
    # Logs détaillés pour le fichier
    Write-Log "=================================================" "INFO"
    Write-Log "=== INFORSUD Technologies - Début d'installation ===" "INFO"
    Write-Log "=================================================" "INFO"
    Write-Log "Date/Heure: $(Get-Date -Format 'dd/MM/yyyy à HH:mm:ss')" "INFO"
    ...
    Write-Log "📋 LOGICIELS SÉLECTIONNÉS POUR INSTALLATION:" "INFO"
    Write-Log "--------------------------------------------" "INFO"
    for ($i = 0; $i -lt $selectedSoftware.Count; $i++) {
        $soft = $selectedSoftware[$i]
        Write-Log "$($i+1). $($soft.Content) (ID: $($soft.Tag))" "INFO" -FileOnly
    }
    Write-Log "" "INFO"
    
    # Affichage simplifié dans l'interface
    Write-Log "=== INFORSUD Technologies - Début d'installation ===" "INFO"
    Write-Log "Nombre de logiciels à installer: $($selectedSoftware.Count)" "INFO"
    Write-Log "Mode silencieux: $($silentCheck.IsChecked)" "INFO"
    Write-Log "=================================================" "INFO"
    ...
    foreach ($checkbox in $selectedSoftware) {
        ...
        Write-Log "   Commande: winget install $softwareId" "INFO" -FileOnly
        ...
        try {
            ...
            if ($process.ExitCode -eq 0) {
                ...
                Write-Log "   Durée: $([math]::Round($duration.TotalSeconds, 1))s | Code retour: 0" "SUCCESS" -FileOnly
                $successCount++
                $successList += "$softwareName (ID: $softwareId)"
            } else {
                ...
                Write-Log "   Durée: $([math]::Round($duration.TotalSeconds, 1))s | Erreur: $errorOutput" "ERROR" -FileOnly
                $errorCount++
                $errorList += "$softwareName (ID: $softwareId) - Code: $($process.ExitCode)"
            }
        } catch {
            $endTime = Get-Date
            $duration = $endTime - $startTime
            Write-Log "❌ Erreur système '$softwareName': $($_.Exception.Message)" "ERROR"
            Write-Log "   Durée: $([math]::Round($duration.TotalSeconds, 1))s | Exception: $($_.Exception.Message)" "ERROR" -FileOnly
            $errorCount++
            $errorList += "$softwareName (ID: $softwareId) - Exception système"
        }
    ...
    # Résumé final détaillé pour le fichier
    Write-Log "" "INFO"
    Write-Log "=================================================" "INFO"
    Write-Log "🏁 BILAN FINAL DE L'INSTALLATION" "INFO"
    Write-Log "=================================================" "INFO"
    Write-Log "Total installé avec succès: $successCount" "SUCCESS"
    Write-Log "Total en échec: $errorCount" "ERROR"
    Write-Log "Taux de succès: $([math]::Round(($successCount/($successCount+$errorCount))*100,1))%" "INFO"
    
    if ($successList.Count -gt 0) {
        Write-Log "" "INFO"
        Write-Log "✅ LOGICIELS INSTALLÉS AVEC SUCCÈS:" "SUCCESS"
        Write-Log "-----------------------------------" "SUCCESS"
        foreach ($success in $successList) {
            Write-Log "  ✓ $success" "SUCCESS" -FileOnly
        }
    }
    
    if ($errorList.Count -gt 0) {
        Write-Log "" "INFO"
        Write-Log "❌ LOGICIELS EN ÉCHEC:" "ERROR"
        Write-Log "----------------------" "ERROR"
        foreach ($error in $errorList) {
            Write-Log "  ✗ $error" "ERROR" -FileOnly
        }
    }
    
    Write-Log "" "INFO"
    Write-Log "📊 RÉSUMÉ TECHNIQUE:" "INFO"
    Write-Log "Heure de fin: $(Get-Date -Format 'dd/MM/yyyy à HH:mm:ss')" "INFO"
    Write-Log "Durée totale: $([math]::Round(((Get-Date) - $startTime).TotalMinutes, 1)) minutes" "INFO"
    Write-Log "Logs détaillés sauvegardés dans: $logPath" "INFO"
    Write-Log "=================================================" "INFO"
    
    # Résumé final simplifié dans l'interface
    Write-Log "=================================================" "INFO"
    Write-Log "🏁 INSTALLATION TERMINÉE - INFORSUD Technologies" "INFO"
    Write-Log "✅ Réussites: $successCount | ❌ Échecs: $errorCount" "INFO"
    Write-Log "💾 Taux de succès: $([math]::Round(($successCount/($successCount+$errorCount))*100,1))%" "INFO"
    
    # Sauvegarde du fichier de log détaillé
    try {
        $logHeader = @"
=== INFORSUD TECHNOLOGIES - RAPPORT D'INSTALLATION DÉTAILLÉ ===
Date de génération: $(Get-Date -Format 'dd/MM/yyyy à HH:mm:ss')
Utilisateur: $env:USERNAME
Machine: $env:COMPUTERNAME
Département: Déploiement Digital (D.D.)
Contact: d.demaere@inforsud-technologies.com
================================================================

"@
        ($logHeader + ($script:detailedLog -join "`n")) | Out-File -FilePath $logPath -Encoding UTF8
        Write-Log "📄 Rapport détaillé sauvegardé: $logPath" "SUCCESS"
    } catch {
...
$window.Add_Closing({
    if ($script:detailedLog.Count -gt 0 -and (Test-Path $logPath)) {
        $result = [System.Windows.MessageBox]::Show("Un rapport d'installation détaillé a été généré par INFORSUD Technologies.`n`nVoulez-vous l'ouvrir pour vérification ?", "Rapport disponible", 'YesNo', 'Question')
        if ($result -eq 'Yes') {
